...
from buzzref.config import CommandlineArgs
    ...
    def hoverMoveEvent(self, event):
        ...
        for corner in self.corners:
            ...
            if self.get_scale_bounds(corner).contains(event.pos()):
                ...
                self.set_cursor(BuzzAssets().cursor_rotate)
                return
        for edge in self.get_flip_bounds():
            if edge['rect'].contains(event.pos()):
                if self.get_edge_flips_v(edge):
                    self.set_cursor(BuzzAssets().cursor_flip_v)
                else:
                    self.set_cursor(BuzzAssets().cursor_flip_h)
                return
